...
#' @description A method to estimate paramaters for spatial threshold autoregressive model
#' @param X a matrix of exogenous regressors
#' @param y a vector of observation on the dependent variable
#' @param W the row normalized spatial weight matrix
#' @param q threshold variable which is assumed exogenous with a continuous distribution
...
#' simulation of tls when dim of regressor is two
#' @param t time of repitition 
#' @param n numbers of random
#' @param beta1 slope coefficient for the spatial lag term if threshold vaviable is less than threshold parameter 
#' @param beta2 slope coefficient for the spatial lag term if threshold vaviable is larger than threshold parameter 
#' @param delta common slope parameters
#' @param gam threshold parameter
#' @return A table of 5%, 50% and 95% of the quantiles of distributions of the estimators of parameters
#' @examples
#' \dontrun{
#'  t<-1000;n<-100;
#'  gamma<-2;beta1<-0.2;beta2<-0.3;delta<-c(1,1);
