...
$\frac{x}{\sigma^2}\exp (-\frac{x^2}{2\sigma^2}), x ≥ 0,\sigma > 0$.
...
My conjecture: When p1=0.5, the empirical distribution of the mixture appears to be bimodal
...
2.The Rayleigh density is 
$\frac{x}{\sigma^2}\exp (-\frac{x^2}{2\sigma^2}), x ≥ 0,\sigma > 0$.Implement a function to generate samples from a Rayleigh(σ) distribution, using antithetic variables. What is the percent reduction in variance of $\frac{X+X'}{2}$ compared with $\frac{X_1+X_2}{2}$ for independent $X_1$, $X_2$?
...
$g(x)=\frac{x^2}{\sqrt{2\pi}}\exp(-\frac{x^2}{2}), x > 1$
Which of your two importance functions should produce the smaller variance in estimating
$\int_{1}^{∞} \frac{x^2}{\sqrt{2\pi}}\exp(-\frac{x^2}{2}) {\rm d}x$
...
f2 can produce the smaller variance in estimating the integration.
...
Answer:
Notice that this integration=$\frac{1}{2}-\int_{0}^{1}\frac{x^2}{\sqrt{2\pi}}\exp(-\frac{x^2}{2}) {\rm d}x$
```{r}
...
1.
Suppose a 95% symmetric t-interval is applied to estimate a mean, but the sample data are non-normal. Then the probability that the confidence interval covers the mean is not necessarily equal to 0.95. Use a Monte Carlo experiment
to estimate the coverage probability of the t-interval for random samples of $\chi^2(2)$ data with sample size n = 20. Compare your t-interval results with the simulation results in Example 6.4. (The t-interval should be more robust to departures from normality than the interval for variance.)
...
```
In this experiment, 91.8% of the intervals contained the population mean, which is close to the 95% coverage under normality. 
Notice that in example 6.4, when the sampled population is $\chi^2(2)$, only 77.3% of the intervals for variance contained the population variance, which is far from the 95% coverage under normality(Page 162 example 6.6). So the t-interval is more robust to departures from normality than the interval for variance.

2.Use Monte Carlo simulation to investigate whether the empirical Type I error rate of the t-test is approximately equal to the nominal significance level $\alpha$, when the sampled population is non-normal. The t-test is robust to mild
departures from normality. Discuss the simulation results for the cases where the sampled population is (i) $\chi^2(1)$, (ii) Uniform(0,2), and (iii) Exponential(rate=1). In each case, test 
$$H_0 : μ = μ_0 \qquad vs \qquad H_0 : μ = μ_0$$,
where $\mu_0$ is the
...
```
We can see that the observed Type I error rate in this simulation is 0.1033,0.0473 and 0.0856 when the sampled population is $\chi^2(1)$, (ii) Uniform(0,2), and (iii) Exponential(rate=1), respectively. When the sampled population is Uniform(0,2), the Estimates of Type I error probability is 0.0504, which is close to the nominal rate $\alpha$ = 0.05. 
...
Answer:
(1)
Becuase the power of a test against a fixed alternative $\theta_1$ ∈ $\Theta_1$ can be estimated by Monte Carlo methods.So I denote the power of method1 as $\pi_1(\theta_1)$, the power of method2 as $\pi_2(\theta_1)$
$$H_0 : \pi_1(\theta_1) = \pi_2(\theta_1) \qquad vs \qquad H_1: \pi_1(\theta_1) \ne\pi_2(\theta_1)$$

(2)Two-sample t-test.
Let $X_{1i}=1$ if we reject $H_0$ in $i_{th}$ experiment for the methed 1 otherwise $X_{1i}=0$.Let $X_{2i}=1$ if we reject $H_0$ in $i_{th}$ experiment for the methed 2, otherwise $X_{2i}=0$. So $\pi_1(\widehat\theta_1)=\frac{\begin{matrix} \sum_{i=1}^n X_{1i} \end{matrix}}{n}$,$\pi_2(\widehat\theta_1)=\frac{\begin{matrix} \sum_{i=1}^n X_{2i} \end{matrix}}{n}$,where n=1000.
In this question, $\pi_1(\widehat\theta_1)$=0.651 and $\pi_1(\widehat\theta_1)$=0.676. We use $\frac{\pi_1(\widehat\theta_1)-\pi_2(\widehat\theta_1)}{se(\pi_1(\widehat\theta_1)-\pi_2(\widehat\theta_1))}$ as the test statistics.When the n is very large, the $\pi_1(\widehat\theta_1)$ and $\pi_2(\widehat\theta_1)$ is Asymptotic normal, so that the statistics is normal.We use Two-sample t-test.

(3)Type1 error $\alpha$,fixed alternative $\theta_1$.

**2021-10-21**
1.
Answer:
Reapeat of Example 6.8:
$$H_0 :\beta_{1,d}=0  \qquad vs \qquad H_1:\beta_{1,d}>0.$$ Assess the Type I error rate for the skewness test of multivariate normality at $\alpha$ = 0.05 based on the asymptotic distribution of$\frac{nb_{1,d}}{6}$ for sample sizes n = 10, 20, 30, 50, 100, and 500.If the sample size is n[i] then $H_0$ is rejected if $\frac{nb_{1,d}}{6} > cv$, where cv is the critical value.
```{r,eval=FALSE}
d<-3 # set dimension=3
...
print(cv)
```
The crit. values for this test is 18.30704.
...
Reapeat of Example 6.10:
We estimate the power of the skewness test for a
sequence of alternatives indexed by $\epsilon$ and plot a power curve for the power of the skewness test against this type of alternative.Set the significance level $\alpha$ = 0.1 and the sample size is n = 30.
```{r,eval=FALSE}
...
The power curve crosses the horizontal line corresponding to $\alpha$ = 0.10 at both endpoints, $\epsilon$ = 0 and $\epsilon$ = 1 where the alternative is normally distributed. For 0 < $\epsilon$ < 1 the empirical power of the test is greater than 0.10 and highest when $\epsilon$ is about 0.15.

**2021-10-28**
1.Refer to Exercise 7.6. Efron and Tibshirani discuss the following example [84,Ch. 7]. The five-dimensional scores data have a 5 × 5 covariance matrix $\Sigma$,with positive eigenvalues $\lambda_1$ > · · · > $\lambda_5$.In principal components analysis,$\theta=\frac{\lambda_1}{\sum_{j=1}^5\lambda_j}$measures the proportion of variance explained by the first principal component.Let $\widehat{\lambda}_1$ > · · ·>$\widehat{\lambda}_5$ be the eigenvalues of $\widehat{\Sigma}$, where $\widehat{\Sigma}$ is the MLE of $\Sigma$. Compute the sample estimate $\widehat{\theta}=\frac{\widehat{\lambda_1}}{\sum_{j=1}^5\widehat{\lambda}_j}$ of $\theta$. Use bootstrap to estimate the bias and standard error of $\widehat{\theta}$.
...
```
The bootstrap estimation of the bias and standard error of $\widehat{\theta}$ are -0.0005 and 0.0477.

2.Refer to Exercise 7.7. Obtain the jackknife estimates of bias and standard error of $\widehat{\theta}$.
```{r}
set.seed(123)
# Jackknife
theta.hat<-obj$t0
...
3.Refer to Exercise 7.7. Compute 95% percentile and BCa confidence intervals for $\widehat{\theta}$.
...
4.Repeat Project 7.A for the sample skewness statistic. Compare the coverage rates for normal populations (skewness 0) and χ2(5) distributions (positive skewness).
```{r,eval=FALSE}
set.seed(123)
#norm
sknorm<-0
...
```
For normal distribution,coverage rates of standard normal bootstrap confidence interval, the basic bootstrap confidence
interval, and the percentile confidence interval are 0.949,0.947 and 0.949.
...
```
For $\chi^2(5)$ distribution,coverage rates of standard normal bootstrap confidence interval, the basic bootstrap confidence
interval, and the percentile confidence interval are 0.866, 0.861, and 0.872.
...
```
The achieved significance level of the permutation test by Spearman rank correlation test is 0.816.The p-value reported by cor.test on the same samples is 0.356.
...
pow
```
Ball test is more powerful than Energy test and nearest NN test.
**2021-11-11**
9.1
...
Answer:
Target distribution is $t(1)$.
Proposal distribution is $N(X_t,\sigma^2)$.
```{r,eval=FALSE}
set.seed(100)
...
```
When $\theta$ = 2, the chain is mixing well and converging to the target
distribution.
```{r,eval=FALSE}
...
```
3.Use the Gelman-Rubin method to monitor convergence of the chain, and run the chain until it converges approximately to the target distribution according to $\widehat{R}$ < 1.2.
```{r,eval=FALSE}
...
```
The trace show that four chains mix well.
```{r,eval=FALSE}
...
When n=2000,The value of $\widehat{R}$ is below 1.1 within 2000 iterations.
**2021-11-18**
11.3
(a)
```{r}
...
```
When n is lager than 50, this sum converges to 1.532164.
...
```
The difference between the root decreases as k increases. The root is approximately 1.73 When k= 100, 500, 1000.Compared to 11.4,the result is similar except when k= 500 and 1000,


2.Use the E-M algorithm to estimate $\lambda$, compare result with the observed data MLE.
```{r,eval=FALSE}
...
The EM estimator is the same as the  MLE based on observed data.
**2021-11-25**
11.1.2.1
```{r}
...
```
这两行代码都表示把分别把x两端的0%, 10%,20%, 50%的数据裁剪后再计算平均值，返回长度为4的列表。
...
```
By qqplot, we can see that the generated random numbers by C++ and R are coincident.The computation time of gibbsC is much less than gibbsR.